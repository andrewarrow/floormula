...
class RoomStore: ObservableObject {
    ...
    #else
    private let savePath = FileManager.default.urls(for: .documentDirectory, in: .userDomainMask)[0].appendingPathComponent("floormula/rooms.json")
    #endif
    
    init() {
        #if os(macOS)
        // Create directory if it doesn't exist
        let directoryPath = FileManager.default.urls(for: .documentDirectory, in: .userDomainMask)[0].appendingPathComponent("floormula")
